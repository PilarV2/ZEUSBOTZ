let handler = async(m, { conn, usedPrefix: _p, command: _c, text, }) => {
    ...
    let sel = `${_c} nya Kan Ada Beberapa Jenis Tuh, Kakak Mau ${_c} Jenis Apa? Pilih Dibawah Ya Kak!\nHarap Perhatikan Menggunakan Bot Dengan Spam Dan Telpon/vc Akan Berakibat Buruk Kepada Anda`
    let rs = `Pilih Disini`
const sections = [ {
	...
	{title: `${_c} loli`, rowId: `${_p + _c}`, description: ``},
	...
	{title: `${_c} Classic`, rowId: `${_p + _c} classic`, description: ``},
...
conn.sendButtonImg(m.chat, im, `Nih Kak ${_c}nya`, wm, 'Next ?', `${_p + _c + ' ' + text}`, m)
