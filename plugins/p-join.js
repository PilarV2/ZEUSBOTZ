let handler = async (m, { conn, text, usedPrefix, args, participants }) => {
  var time = db.data.users[m.sender].lastjoin + 864224700000
  if (new Date - db.data.users[m.sender].lastjoin < 864224700000) throw `Kamu sudah menggunakan limit invite bot harian hari ini\ntunggu selama ${msToTime(time - new Date())} lagi`
  var linkRegex = /chat.whatsapp.com\/([0-9A-Za-z]{20,24})/i
  var delay = time => new Promise(res => setTimeout(res, time))
 
  var name = m.sender
  var ftroli = {key: {fromMe: false,"participant":"0@s.whatsapp.net", "remoteJid": "6289523258649-1604595598@g.us"}, "message": {orderMessage: {itemCount: 0,status: 200, surface: 200, message:  `Hello Everyone👋🏻\nSilahkan Ketik #menu\nUntuk Memulai Perintah`, orderTitle: 'Centauri', sellerJid: '0@s.whatsapp.net'}}}
  ...
  if (e.length) await conn.reply(res, `Bot Telah Di Invite Ke Grup ${await conn.getName(res)} Thanks dah invite Gua *${m.name}*`, ftroli, {
    mentions: d
     }).then(async () => {
     await delay(5000)
     }).then( async () => {
     await conn.reply(res, `Bot akan keluar otomatis setelah *${msToDate(global.db.data.chats[res].expired - now)}*`, 0)
